...
## Backlog (continued)
- [ ] **Codify the scan→deploy pipeline as a reusable workflow** (idea 2026-07-06, deferred — "for now log it"). Chain: `ingest-scan.py` → `enrich.py` → resolve dark EANs → `build_price_sheet.py` (fill prices) → `allegro_draft.py create` → activate. Form undecided: (A) project slash-command skill + thin orchestrator [recommended — "agent oversees scripts", low token], (B) `scripts/pipeline.py` orchestrator + runbook only [you drive], (C) multi-agent Workflow tool [token-heavy, wrong shape here]. Design notes to bake in: **3 gates never auto-crossed** — agent gate (resolve dark→overrides) + 2 human gates (fill prices; **activate = the sell line**); a **status/funnel command** (scanned→identified→priced→drafted→active); a **pre-flight compliance check** `allegro_draft.py check` (GPSR/delivery/images present; hard-block the manual/scraped-image wave from `create`); keep the agent step tiny/last-resort; dry-run→backup→apply for every live write. See [[allegro-listing]].
